...
public class userController {
    @Autowired
    private com.example.demo.service.policeService policeService;
    ...
    public user query(String account){
        ...
        if(user==null){
          userService  userService=(userService)factoryBean2.getObject();
            user=userService.queryUserByAccount(account);
        }
        return user;
    }
    ...
    public ModelAndView login(@RequestParam("account") String account,
        ...
        if(!StringUtils.isEmpty(account) &&!StringUtils.isEmpty(password)) {
            if(type.equals("车主")) {
                ...
                try {
                    if (range >= 1 && range <= 3) {
                        System.out.println("北京客户端调用上海远程服务");
                        ...
                        System.out.println("北京客户端调用广东远程服务");
